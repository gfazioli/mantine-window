...
Enable state persistence to save the window's position and size in localStorage by setting `persistState={true}` (default is `false`). When enabled, the window will remember its position and size across page refreshes. Set `persistState={false}` to disable this behavior and always start at the default position and size.
...
- **Larger hit area**: Side handles span the full edge length
- **Better UX**: No need for precision to find the resize handle
- **Corner handles preserved**: Diagonal resizing still available at 14x14px corners
