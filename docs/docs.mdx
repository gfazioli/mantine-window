...
Each `Window` inside a `Window.Group` must have a unique `id` prop. The group automatically sets `withinPortal={false}` and `position: relative` on its container.

<Demo data={demos.group} />

### Layout Presets

Click the green button in any window header to access layout options. Available layouts:

- **Snap left / Snap right**: Fill half the container
- **Fill**: Maximize the window to fill the entire container
- **Tile all**: Automatically arrange all visible windows in a grid

The `onLayoutChange` callback notifies you when a layout is applied.
