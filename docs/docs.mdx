...
You can use viewport units (`vw` for horizontal, `vh` for vertical) to set responsive positions that adapt to the browser window size. This ensures the window maintains proportional positioning regardless of screen size.

<Demo data={demos.viewportPosition} />

## Viewport-based Size

Use viewport units (`vw` for width, `vh` for height) to create responsive window sizes that scale proportionally with the viewport. This is ideal for creating layouts that adapt to different screen sizes.
