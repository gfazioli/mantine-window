...
By default, the window's position and size are persisted in localStorage. You can disable this behavior by setting `persistState={false}`. This is useful when you want the window to always start at the same position and size.

<Demo data={demos.noPersistence} />
